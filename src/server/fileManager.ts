...
export function readFileSafe(full: string) {
  if (!fs.existsSync(full)) throw new Error("not found");
  return fs.readFileSync(full, "utf8");
}

/**
 * Recursively lists a tree from root and returns a simple JSON structure
 */
export function listTree(root: string) {
  if (!fs.existsSync(root)) return { type: "dir", name: path.basename(root), path: "", children: [] };
  ...
  function walk(p: string): any {
    ...
    if (stat.isDirectory()) {
      ...
      return {
        ...
        name,
        path: rel,
        children: entries.map((c) => walk(path.join(p, c))),
      };
    }
